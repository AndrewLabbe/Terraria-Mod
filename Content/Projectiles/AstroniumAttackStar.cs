...
using VoyagerMod.Content.Graphics.Renderers;
using VoyagerMod.Content.Utilities;

namespace VoyagerMod.Content.Projectiles
{
    public class AstroniumAttackStar : ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            ...
            if (Owner.ownedProjectileCounts[ModContent.ProjectileType<AstroniumAttackHoldout>()] <= 0 && Time >= 2f)
            {
                Projectile.Kill();
                return;
            }
        ...
        {
            float damageFactor = MathHelper.Lerp(1f, AstroniumStaff.MaxDamageBoostFactor, Utils.GetLerpValue(0f, AstroniumStaff.MaxDamageBoostTime, Time, true));
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                if (enemyIsInShape)
                {
                    ...
                    if (Main.myPlayer == Projectile.owner)
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Main.npc[i].Center, Vector2.Zero, ModContent.ProjectileType<AstroniumAttackStrike>(), damage, 0f, Projectile.owner, i);
                    }
                }
            }
        }
