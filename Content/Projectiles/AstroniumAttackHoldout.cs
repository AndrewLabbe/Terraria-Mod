...
{
    public class AstroniumAttackHoldout : ModProjectile
    {
        public override LocalizedText DisplayName => VoyagerUtils.GetItemName<AstroniumStaff>();
        public Player Owner => Main.player[Projectile.owner];
        public const float AimResponsiveness = 0.72f;

        public override string Texture => "VoyagerMod/Content/Items/AstroniumStaff";
        ...
        public override void AI()
        {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                // Create the star.
                int attackType = ModContent.ProjectileType<AstroniumAttackStar>();
                if (Owner.ownedProjectileCounts[attackType] == 0)
                {
                    if (Projectile.ai[0] >= 0f && Owner.CheckMana(Owner.ActiveItem(), -1, true, false))
                    {
                        SoundEngine.PlaySound(AstroniumStaff.UseSound, Owner.Center);
        ...
        private void UpdatePlayerVisuals()
        {
            ...
            Owner.itemRotation = VoyagerUtils.WrapAngle90Degrees(Projectile.rotation);
        }
